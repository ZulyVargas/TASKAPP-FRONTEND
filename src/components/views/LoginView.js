...
function LoginView() {
  ...
  const getUser = async (tk) => {
    ...
    getRequests
      .UseFetchGETUsingToken(request)
      .then((res) =>
        Object.prototype.hasOwnProperty.call(res.data, "id")
          ? (setUser(res.data), setAuth({ auth: true }))
          : setError({ error: true })
      );
  };
  ...
  useEffect(() => {
    ...
    try {
      if (submit.submit) {
        UseFetchPOST(request).then((res) =>
          Object.prototype.hasOwnProperty.call(res.data, "token")
  ...
  return (
    <Grid container id="GridLogin" component="main">
      ...
      >
        ...
        <h2>Login Account</h2>
        {error.error && (
          <Alert severity="warning">Incorrect email or password</Alert>
        )}
        <form>
          <TextField
            id="mail"
