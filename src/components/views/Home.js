...
function Home() {
    ...
    useEffect(() =>{
        try {
            if (!(inHome.inHome)){ 
                console.log('in home use effect ' +userInfo.name);
                const urlGetTasksById = 'http://localhost:8081/api/v1/task/tasks/'+ userInfo.id;
                const request = {'url': urlGetTasksById};
                const response = getRequests.UseFetchGET(request)
                .then(res => (res.data.length > 0  ? setTasks(res.data) : console.log('No ok')));  
        ...
        <div>
            ...
            <h1>👋 Hello, Welcome {userInfo.name} !</h1>
                <Grid className='tasks' rowSpacing={3} justifyContent="center" columnSpacing={1} container>
                    {tasks.map((task) =>
                    (<Grid item xs="auto" wrap="nowrap" container key={task.id}><Task key={task.id} name = {task.name} 
                                        description ={task.description}
                                        status={task.status}
                                        dueDate={task.dueDate}></Task></Grid>))}
                </Grid>
